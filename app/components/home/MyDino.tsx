...
import { liveDinoDemo, type LiveDinoDemo } from "@/app/data/myDino";
...
export default function MyDino() {
  ...
  const [dino, setDino] = useState<LiveDinoDemo | null>(null);
  const [dinoMsg, setDinoMsg] = useState<string | null>(null);
  const [dinoError, setDinoError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      try {
        ...
        setLoggedIn(true);
        setWallet({
          coins: meJson.user.coins,
          marks: meJson.user.marks,
          tierLabel:
            meJson.user.tierLabel ||
            meJson.user.patreonTier ||
            "Sin membresía",
        });

        const dinoRes = await fetch("/api/player/dino", { cache: "no-store" });
        const dinoJson = (await dinoRes.json()) as DinoResponse;
        if (cancelled) return;
        if (dinoJson.ok && dinoJson.dino) {
          setDino(dinoJson.dino);
          setDinoMsg(null);
          setDinoError(null);
        } else {
          setDino(null);
          setDinoMsg(dinoJson.message || null);
          setDinoError(dinoJson.error || null);
        }
      } catch {
  ...
  const panelDino = dino || (demoOpen ? liveDinoDemo : null);
  const showRealPanel = loggedIn && !!dino;
  const showDemoPanel = !loggedIn && demoOpen;
              ...
              {!loggedIn && !demoOpen ? (
                ...
                <div className="flex flex-1 flex-col items-center justify-center gap-4 py-8 text-center">
                  ...
                  <p className="max-w-sm text-sm leading-relaxed text-zinc-400">
                    {dinoMsg ||
                      dinoError ||
                      "Entra al servidor y spawnea un dinosaurio para ver tus vitales aquí."}
                  </p>
                  <a
                    href={`steam://connect/${siteConfig.serverIp}`}
                    className="rounded-full bg-red-600 px-6 py-3 text-xs font-black uppercase tracking-wider text-white"
                  >
                    Entrar al server
                  </a>
                </div>
              ) : panelDino && (showRealPanel || showDemoPanel) ? (
                <>
                  <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    ...
                    {showDemoPanel ? (
                      ...
                      >
                        {panelDino.online ? "En juego" : "Offline"}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="relative mx-auto mb-6 flex h-28 w-28 items-center justify-center">
                    <div
                      ...
                      style={{
                        background: `conic-gradient(#dc2626 ${panelDino.growth * 3.6}deg, rgba(255,255,255,0.08) 0)`,
                      }}
                    />
                  ...
                  <div className="mb-4 grid grid-cols-2 gap-2 text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                    <span>Zona: {panelDino.zone}</span>
                    <span>Loc: {panelDino.location}</span>
                    <span>
                      K/D: {panelDino.kills}/{panelDino.deaths}
                    </span>
                    <span>Dieta: {panelDino.diet}</span>
                  </div>

                  <div className="space-y-4">
                    {panelDino.vitals.map((vital) => (
                      <VitalBar
                        key={vital.id}
                        label={vital.label}
                        value={vital.value}
